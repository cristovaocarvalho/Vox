...
https = __toESM(https);
//#region electron/modules/recorder.ts
var AudioRecorder = class extends events.EventEmitter {
	isRecording = false;
	chunks = [];
	vadThreshold = .02;
	...
	processAudioChunk(chunk) {
		...
		if (this.autoStopOnSilence) {
			...
			} else if (this.hasSpoken && !this.silenceTimer) this.silenceTimer = setTimeout(() => {
	...
	stopRecording() {
		...
		this.chunks = [];
		if (pcmData.length === 0) return Buffer.alloc(0);
	...
	calculateRmsEnergy(buffer) {
		...
		for (let i = 0; i < sampleCount; i++) {
			const normalized = buffer.readInt16LE(i * 2) / 32768;
...
//#endregion
//#region electron/modules/stt.ts
var GROQ_API_KEY$1 = "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";
var GROQ_STT_ENDPOINT = "https://api.groq.com/openai/v1/audio/transcriptions";
...
//#endregion
//#region electron/modules/corrector.ts
var GROQ_API_KEY = "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";
var GROQ_CHAT_ENDPOINT = "https://api.groq.com/openai/v1/chat/completions";
var DEFAULT_LLM_MODEL = "openai/gpt-oss-20b";
async function correctTranscription(text) {
	...
	try {
		...
		const corrected = (await response.json()).choices?.[0]?.message?.content?.trim();
		if (corrected) {
...
//#endregion
//#region electron/modules/injector.ts
...
async function injectText(text, _mode = "clipboard", delayMs = 100, hwnd) {
	...
	try {
		...
		(0, child_process.execFile)("powershell", [
			"-NoProfile",
			"-WindowStyle",
			"Hidden",
			"-Command",
			...
			if (err) console.error("[Injector] Erro ao colar texto via PowerShell:", err);
			else console.log("[Injector] Texto colado no cursor com sucesso!");
		});
	} catch (err) {
...
//#endregion
//#region electron/modules/downloader.ts
	...
	return new Promise((resolve, reject) => {
		https.default.get(url, (res) => {
			if (res.statusCode === 301 || res.statusCode === 302) {
				if (res.headers.location) return downloadBinary(res.headers.location, targetPath).then(resolve).catch(reject);
			}
			if (res.statusCode !== 200) return reject(/* @__PURE__ */ new Error(`Falha ao baixar binário: HTTP ${res.statusCode}`));
			const fileStream = fs.default.createWriteStream(targetPath);
			res.pipe(fileStream);
			...
			fileStream.on("error", (err) => {
				fs.default.unlink(targetPath, () => reject(err));
			});
		}).on("error", (err) => {
			reject(err);
		});
...
async function ensureExecutable() {
	const possiblePaths = [
		path.default.join(process.cwd(), "resources", "binaries", "yt-dlp.exe"),
		path.default.join(process.cwd(), "resources", "binaries", "yt-dlp"),
		path.default.join(__dirname, "..", "..", "resources", "binaries", "yt-dlp.exe"),
		path.default.join(__dirname, "..", "..", "resources", "binaries", "yt-dlp"),
		"yt-dlp"
	];
	for (const p of possiblePaths) if (p !== "yt-dlp" && fs.default.existsSync(p)) return p;
	const targetDir = path.default.join(process.cwd(), "resources", "binaries");
	if (!fs.default.existsSync(targetDir)) fs.default.mkdirSync(targetDir, { recursive: true });
	const targetFile = path.default.join(targetDir, "yt-dlp.exe");
	if (!fs.default.existsSync(targetFile)) {
	...
	return fs.default.existsSync(targetFile) ? targetFile : "yt-dlp";
}
async function getVideoInfo(url, cookiesFromBrowser) {
	...
	return new Promise((resolve) => {
		...
		let stderr = "";
		try {
			const proc = (0, child_process.spawn)(executable, args);
			...
			proc.on("close", (code) => {
				...
				if (cookiesFromBrowser && cookiesFromBrowser !== "none" && /cookie database|Could not copy/i.test(stderr)) return getVideoInfo(url, "none").then(resolve);
				resolve({
...
async function downloadAudio(options) {
	...
	const executable = await ensureExecutable();
	const baseFileName = `vox_media_${Date.now()}`;
	const args = [
		"-f",
		"ba/b/best",
		"--no-playlist",
		...
		"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36",
		"-o",
		path.default.join(options.outputDir, `${baseFileName}.%(ext)s`),
		options.url
	];
	if (options.cookiesFromBrowser && options.cookiesFromBrowser !== "none") args.push("--cookies-from-browser", options.cookiesFromBrowser);
	return new Promise((resolve, reject) => {
		let stderrText = "";
		try {
			const proc = (0, child_process.spawn)(executable, args);
			proc.stdout.on("data", (chunk) => {
				const match = chunk.toString().match(/\[download\]\s+(\d+\.\d+)%\s+of\s+\S+\s+at\s+(\S+)\s+ETA\s+(\S+)/);
			...
			proc.on("close", (code) => {
				try {
					const downloadedFile = fs.default.readdirSync(options.outputDir).find((f) => f.startsWith(baseFileName));
					if (downloadedFile) {
						const fullPath = path.default.join(options.outputDir, downloadedFile);
				...
				if (/cookie database|Could not copy|cookies/i.test(stderrText) && options.cookiesFromBrowser && options.cookiesFromBrowser !== "none") {
				...
				if (code !== 0) {
					...
					if (cleanError) return reject(/* @__PURE__ */ new Error(`Falha no download: ${cleanError}`));
					return reject(/* @__PURE__ */ new Error(`yt-dlp não conseguiu baixar a mídia (Código ${code}). Verifique se a URL é pública e acessível.`));
				}
				reject(/* @__PURE__ */ new Error("O arquivo de mídia não foi encontrado na pasta Downloads."));
			});
			proc.on("error", (err) => {
				reject(/* @__PURE__ */ new Error(`Erro ao executar o downloader (${err.message}).`));
			});
		} catch (err) {
...
var downloader_default = {
	detectPlatform,
	getVideoInfo,
	downloadAudio
};
//#endregion
//#region electron/modules/db.ts
...
function initDatabase() {
	try {
		...
		const dbPath = path.default.join(userDataPath, "vox_settings.db");
		fallbackFile = path.default.join(userDataPath, "vox_settings.json");
		if (Database) {
      ...
      `);
		...
		} else console.log("[DB] Usando fallback de dados em:", fallbackFile);
	} catch (err) {
...
function getSetting(key, defaultValue = "") {
	try {
		...
		if (fallbackFile && fs.default.existsSync(fallbackFile)) {
			const data = JSON.parse(fs.default.readFileSync(fallbackFile, "utf-8"));
...
function setSetting(key, value) {
	try {
      ...
      `).run(key, value);
		...
		if (fallbackFile) {
			let data = {};
			if (fs.default.existsSync(fallbackFile)) try {
				data = JSON.parse(fs.default.readFileSync(fallbackFile, "utf-8"));
			} catch {
				data = {};
			}
			data[key] = value;
			fs.default.writeFileSync(fallbackFile, JSON.stringify(data, null, 2), "utf-8");
		}
	} catch (err) {
...
//#endregion
//#region electron/modules/wakeword.ts
/**
...
var WakeWordDetector = class extends events.EventEmitter {
	active = false;
	session = null;
	sensitivity = .5;
	lastTriggerTime = 0;
	...
	async init(modelPath, sensitivity = .5) {
		this.sensitivity = sensitivity;
		const defaultModelPath = path.default.join(process.cwd(), "resources", "models", "wakeword", "vox.onnx");
		const finalPath = modelPath || defaultModelPath;
		if (ort && fs.default.existsSync(finalPath)) try {
		...
		else console.log("[WakeWord] Escuta ativa para a palavra-chave \"Vox\" inicializada em modo VAD adaptativo (aguardando modelo vox.onnx em resources/models/wakeword/)");
	}
	start() {
		this.active = true;
		this.audioBuffer = [];
	...
	setSensitivity(value) {
		this.sensitivity = Math.max(.1, Math.min(1, value));
	}
	isListening() {
		return this.active;
	}
	...
	async evaluateFrame(samples) {
		if (Date.now() - this.lastTriggerTime < this.cooldownMs) return;
		if (this.session && ort) try {
			...
			const outputTensor = (await this.session.run(feeds))[this.session.outputNames[0] || "output"];
			if (outputTensor && outputTensor.data) {
				const score = outputTensor.data[0];
				if (score >= this.sensitivity) this.triggerDetection(score);
			}
		} catch (err) {
			...
			if (rms > .25 * (1.1 - this.sensitivity)) this.triggerDetection(rms);
		}
	}
...
var { app, BrowserWindow, ipcMain, globalShortcut, screen, dialog, Tray, Menu, nativeImage } = require("electron");
var mainWindow = null;
var dockWindow = null;
var tray = null;
var targetWindowHwnd = null;
function captureActiveWindow() {
	try {
		targetWindowHwnd = (0, child_process.execFileSync)("powershell", [
			"-NoProfile",
			"-WindowStyle",
			"Hidden",
			"-Command",
...
var getDevUrl = () => process.env["ELECTRON_RENDERER_URL"] || process.env["VITE_DEV_SERVER_URL"];
...
function createDockWindow() {
	...
	const devUrl = getDevUrl();
	if (devUrl) dockWindow?.loadURL(`${devUrl}#/dock`);
	else dockWindow?.loadFile(path.default.join(__dirname, "../renderer/index.html"), { hash: "dock" });
	positionDockWindow();
}
function showDock() {
	if (!dockWindow) return;
...
function setupIpcHandlers() {
	recorder.on("energy", (data) => {
		if (dockWindow && !dockWindow.isDestroyed()) dockWindow.webContents.send("vox:volume-update", data);
		if (mainWindow && !mainWindow.isDestroyed()) mainWindow.webContents.send("vox:volume-update", data);
	});
	...
	ipcMain.handle("vox:stop-recording", async (_event, audioData) => {
		...
		} else buffer = recorder.stopRecording();
		...
		if (result.text) {
			...
			if (mainWindow && !mainWindow.isDestroyed()) mainWindow.webContents.send("vox:transcript-result", result.text);
		}
		return result;
	});
	...
	ipcMain.on("vox:audio-chunk", (_event, chunk) => {
		if (chunk) {
			...
			if (wakewordDetector.isListening()) wakewordDetector.processAudioChunk(buf);
		}
	});
	ipcMain.handle("vox:transcribe-chunk", async (_event, audioData) => {
		if (!audioData || audioData.byteLength < 1e3) return { text: "" };
		try {
			const text = (await transcribeAudio(Buffer.from(audioData))).text || "";
			if (text && !text.startsWith("[Erro")) {
				if (mainWindow && !mainWindow.isDestroyed()) mainWindow.webContents.send("vox:partial-transcript", text);
				if (dockWindow && !dockWindow.isDestroyed()) dockWindow.webContents.send("vox:partial-transcript", text);
			}
			return { text };
		} catch (error) {
	...
	ipcMain.handle("vox:get-video-info", async (_event, url, cookiesFromBrowser) => {
		try {
			return await downloader_default.getVideoInfo(url, cookiesFromBrowser);
		} catch (err) {
			console.error("[Main] Erro ao obter info do vídeo:", err);
			return {
				title: "Vídeo Desconhecido",
				duration: 0,
				thumbnail: "",
				platform: "unknown"
			};
	...
	ipcMain.handle("vox:download-audio", async (_event, url, cookiesFromBrowser) => {
		try {
			...
			return await downloader_default.downloadAudio({
				url,
				outputDir: downloadsDir,
				cookiesFromBrowser,
				onProgress: (pct, speed, eta) => {
					if (mainWindow && !mainWindow.isDestroyed()) mainWindow.webContents.send("vox:download-progress", {
						pct,
						speed,
						eta
					});
	...
	ipcMain.handle("vox:transcribe-media", async (_event, options) => {
		try {
			if (!options.audioPath || !fs.default.existsSync(options.audioPath)) return { text: "[Erro: Arquivo de áudio não encontrado]" };
			let text = (await transcribeAudio(fs.default.readFileSync(options.audioPath))).text || "";
			if (text && !text.startsWith("[Erro")) text = await correctTranscription(text);
			return { text };
		} catch (err) {
	...
	ipcMain.handle("vox:delete-file", async (_event, filePath) => {
		try {
			if (filePath && fs.default.existsSync(filePath)) {
	...
	ipcMain.handle("vox:save-settings", async (_event, settings) => {
		try {
			if (settings && typeof settings === "object") {
				for (const [key, value] of Object.entries(settings)) setSetting(key, String(value));
				if (settings.wakeWordSensitivity) wakewordDetector.setSensitivity(parseFloat(settings.wakeWordSensitivity));
				if (settings.wakeWordEnabled === "true") wakewordDetector.start();
				else if (settings.wakeWordEnabled === "false") wakewordDetector.stop();
			}
			return { success: true };
		} catch (err) {
...
function toggleDockWindow() {
	...
	if (isVisible) hideDock();
	else {
		captureActiveWindow();
		showDock();
	}
	if (mainWindow && !mainWindow.isDestroyed()) mainWindow.webContents.send("vox:toggle-recording", !isVisible);
}
var f9ReleaseTimer = null;
function handleGlobalPushToTalk() {
	if (!dockWindow) return;
	if (!dockWindow.isVisible()) {
		captureActiveWindow();
		showDock();
		if (mainWindow && !mainWindow.isDestroyed()) mainWindow.webContents.send("vox:toggle-recording", true);
	}
	if (f9ReleaseTimer) clearTimeout(f9ReleaseTimer);
	f9ReleaseTimer = setTimeout(() => {
		hideDock();
		if (mainWindow && !mainWindow.isDestroyed()) mainWindow.webContents.send("vox:toggle-recording", false);
		f9ReleaseTimer = null;
	}, 180);
}
app.whenReady().then(async () => {
	...
	if (settings.wakeWordEnabled !== "false") wakewordDetector.start();
	try {
	...
	const iconPath = app.isPackaged ? path.default.join(process.resourcesPath, "favicon.png") : path.default.join(app.getAppPath(), "src/favicon.png");
